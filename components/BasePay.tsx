...
// Dynamically import Base SDK components to avoid SSR issues
const BasePayButton = dynamic(
  () => import('@base-org/account-ui/react').then(mod => ({ default: mod.BasePayButton })),
  { 
    ssr: false,
    loading: () => (
      <div className="w-full h-20 rounded-base btn-3d bg-blue-600 flex items-center justify-center text-white font-semibold">
        <span>Loading Base Pay...</span>
      </div>
    )
  }
);
...
export function BasePay() {
  ...
  const handlePay = async () => {
    try {
      ...
      // Dynamically import pay functions when needed
  ...
  return (
    <div className="mb-4">
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-black mb-2" style={{ fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>
            Payment Amount (USD)
          </label>
          <div className="relative">
            ...
            <input
              ...
              step="0.01"
              min="0.01"
              className="w-full pl-8 pr-3 py-2 border border-gray-300 bg-white rounded-lg text-black placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder="5.00"
        ...
        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="text-sm font-medium text-blue-900 mb-1">Payment Recipient</div>
          <div className="font-mono text-xs text-gray-600 mb-1">{recipient}</div>
          <div className="text-xs text-blue-700">dylsteck.base.eth</div>
        </div>
...
// Grid button version that matches token button dimensions exactly
export function BasePayGridButton() {
  const [isProcessing, setIsProcessing] = useState(false);
  ...
  const handlePay = async () => {
    try {
      ...
      const payResult = result as PayResult;
      const transactionId = payResult?.id || payResult?.transactionId;
      ...
      if (transactionId) {
        ...
        if (status === 'completed') {
          ...
          // For simplicity, assume success if we get a transaction ID
          setPaymentStatus('success');
        }
      }
    } catch (error) {
  ...
  return (
    <div className="w-full h-auto">
      ...
      {/* Custom status overlays that match the token button styling */}
      {paymentStatus === 'success' && (
        <div className="w-full h-20 rounded-base btn-3d bg-green-600 hover:opacity-80 transition-opacity flex items-center justify-center text-white font-semibold">
          <div className="flex items-center gap-2">
            <span className="text-lg">✅</span>
            <div className="text-center">
              <div className="text-sm font-bold">Payment Sent!</div>
              <div className="text-xs opacity-90">$5 USDC Paid</div>
            </div>
          </div>
        </div>
      )}
      ...
      {paymentStatus === 'failed' && (
        <div className="w-full h-20 rounded-base btn-3d bg-red-600 hover:opacity-80 transition-opacity flex items-center justify-center text-white font-semibold cursor-pointer" onClick={() => setPaymentStatus('idle')}>
          <div className="flex items-center gap-2">
            <span className="text-lg">❌</span>
            <div className="text-center">
              <div className="text-sm font-bold">Payment Failed</div>
              <div className="text-xs opacity-90">Click to Retry</div>
            </div>
          </div>
        </div>
      )}
      ...
      {isProcessing && (
        <div className="w-full h-20 rounded-base btn-3d bg-blue-600 hover:opacity-80 transition-opacity flex items-center justify-center text-white font-semibold">
          <div className="flex items-center gap-2">
